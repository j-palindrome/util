...
export default function Brush({
  ...
  storageTexture.minFilter = storageTexture.magFilter = THREE.LinearFilter
  storageTexture.type = THREE.FloatType
  const dimensionsU = uniform(lastData.dimensions, 'vec2')

  // @ts-ignore
  const textureLoadFix = wgslFn<{
    uv: ReturnType<typeof ivec2>
    tex: ReturnType<typeof texture>
  }>(/*wgsl*/ `
    fn loadTexture(uv: vec2<i32>, tex: texture_2d<f32>) -> vec4<f32> {
  ...
  useMemo(() => {
    //  nodeVar1 = textureLoad( nodeUniform1, vec2u( tsl_coord_clampS_clampT( ( object.nodeUniform2 * vec3<f32>( nodeVarying1, 1.0 ) ).xy ) * vec2f( nodeVar2 ) ) ); <-- this Goes Away without filtering being set to nearest. Otherwise it goes to a bogus textureLoad function which I can't seem to replace.
    // this simply isn't working. I've tried everything.
    // Disable the filtering...
    // const storageTexNode = uniform(storageTexture, 'texture_2d<f32>')
